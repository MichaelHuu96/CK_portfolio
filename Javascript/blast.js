...
function blast(section){
  
  $(section+ " .preblasted").css("opacity", 1);

  $(section + " .blast_container h1").blast({
    delimiter: "character",
    tag: "span"
  });

  var a = 0;
  $(section + " .blast_container .blast").each(function() {
    if (a == 300) {
      a = 400;
    }
    ...
    if (a == 400) {
      setTimeout(function() {
        $(section + " .blast_container h1 img").addClass("animated rotateIn");
      }, 500);
    }
...
function rubbereffect(section){
  $("#"+entry.target.id +".blast_container .flat-button").mouseenter(function() {
    var el = $(this);
...
const observer = new IntersectionObserver(entries => {
  ...
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      ...
      if (visited_sections.indexOf("#" + entry.target.id) !== -1) {
        return
      }
      visited_sections.push("#" + entry.target.id);
              //codeblock
      blast("#"+entry.target.id);
      setTimeout(function() {
        $("#"+entry.target.id+" .blast_container .blast").removeClass("animated bounceIn");
        $("#"+entry.target.id+" .blast_container .blast").css("opacity", 1);
      
        $("#"+entry.target.id+" .blast_container .blast").mouseenter(function() {
          var el = $(this);
      ...
      setTimeout(function() {
        $("#"+entry.target.id+" .blast_container .flat-button").addClass("animated bounceIn");
        $(
          "#"+entry.target.id+" .blast_container .flat-button"
        ).one(
          "webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend",
          function() {
            $("#"+entry.target.id+" .blast_container .flat-button").removeClass("animated bounceIn");
            $("#"+entry.target.id+" .blast_container .flat-button").css("opacity", 1);
          }
        );
      }, 1000);
              //codeblock
